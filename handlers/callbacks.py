...
async def handle_callbacks(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    logger.info("shopping.callback.received user_id=%s data=%s", user_id, data)
    ...
    try:
        ...
        logger.exception("shopping.callback.error user_id=%s data=%s", user_id, data)
        await query.message.reply_text(
            "⚠️ Не получилось обработать кнопку. Откройте раздел «Покупки» заново.",
        )
